...
def ____evaluate_one_by_one(model, full_data, train_data, train_mask, test_mask):
    ...
    node_mapping = {old_id.item(): new_id for new_id, old_id in enumerate(train_indices)}
    ...
    with torch.no_grad():
        for test_index in trange(len(test_indices)):
            ...
            pred = out[-1].argmax(dim=0).item()
            true_label = sub_data.y[-1].item()
...
def my_subgraph(full_data, train_data, train_indices, test_node_id, node_mapping):
    ...
    for i in range(test_edges.size(1)):
        test_edges[0, i] = node_mapping.get(test_edges[0, i].item(), train_data.num_nodes)
        test_edges[1, i] = node_mapping.get(test_edges[1, i].item(), train_data.num_nodes)
    ...
    return Data(
        ...
        edge_index=edge_index_plus,
        edge_attr=edge_attr_plus,
        y=y_plus
    )
...
def evaluate_one_by_one(model, full_data, train_mask, test_mask, disable=False):
    ...
    with torch.no_grad():
        for test_index in trange(len(test_indices), disable=disable):
            ...
            unknown_label = torch.tensor([1. / x_input.shape[1] for _ in range(x_input.shape[1])]).type(torch.float).to(
                device)
...
def change_input(x_input, q=10):
    ...
    unknown_label = torch.tensor([1./x_input.shape[1] for _ in range(x_input.shape[1])]).type(torch.float).to(device)
